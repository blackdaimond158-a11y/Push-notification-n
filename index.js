...
<title>Wevlo Push — Test Device</title>
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .btn{
    ...
    border:none;padding:12px 16px;border-radius:8px;font-size:14px;cursor:pointer;margin-top:18px;
...
<div class="wrap">
  ...
  <div class="card hidden" id="connectedCard" style="display:none;">
    <h2>ডিভাইস টোকেন</h2>
...
async function connectDevice(){
  ...
  if(!pushAppId){ setMsg('App ID দিন', false); return; }
  if(!vapidKey){ setMsg('VAPID Key দিন', false); return; }
  ...
  try{
    ...
    if(!data.success){
      setMsg('সার্ভারে রেজিস্টার ব্যর্থ: ' + data.error, false);
      btn.disabled = false;
      return;
    }
    ...
    setMsg('কানেক্ট করতে ব্যর্থ: ' + e.message, false);
  }finally{
    btn.disabled = false;
  }
}
...
app.post('/register-token', async (req, res) => {
  ...
  try {
    ...
    console.log(`[${appId}] Token registered`);
...
app.post('/send-all', async (req, res) => {
  ...
  try {
    ...
    if (snap.empty) return res.json({ success: false, error: 'No tokens found' });
